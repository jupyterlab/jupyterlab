 ...
 *
 * @param replace - If URLs should be replaced.
 *
...
function autolink(
  ...
  while (null != (match = webLinkRegex.exec(content)) && replace) {
...
export function renderText(options: renderText.IRenderOptions): Promise<void> {
  ...
  if (preTextContent) {
    ...
    const linkedNodes = autolink(preTextContent, sanitizer.getAutolink());
