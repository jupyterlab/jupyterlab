...
//       }
//       cell.addClass(CONSOLE_CELL_CLASS);
//       // Executed cells are read-only — remove them from tab order entirely
//       cell.node.tabIndex = -1;
//       cell.node.querySelectorAll<HTMLElement>(
//         '[tabindex], input, button, textarea, select, a'
//       ).forEach(el => el.setAttribute('tabindex', '-1'));
...
//    */
// /**
//    * Apply accessibility attributes to a banner cell.
//    */
//   private _applyBannerAccessibility(banner: RawCell): void {
//     banner.node.tabIndex = 0;
//     banner.node.setAttribute('role', 'note');
//     banner.node.setAttribute('aria-label', 'Kernel Banner');
//     // Keep inner elements non-tabbable — the banner node itself is the tab stop
//     banner.node.querySelectorAll<HTMLElement>(
//       'input, button, textarea, select, a, [tabindex]'
//     ).forEach(el => el.setAttribute('tabindex', '-1'));
//   }
...
//   // Make the console container focusable so focus is never lost
// // Console root: focusable as last-resort fallback only, not in tab order
//   node.tabIndex = -1;

//   // === Tab order: toolbar (external) → banner → content area → prompt editor → toolbar ===

//   // Banner: tabIndex=0 so Tab reaches it, read-only so user can review it
//   // Banner accessibility is applied in addBanner() at creation time
//   // because the banner is created after onAfterAttach fires.

//   // Content panel: tabIndex=0, arrow keys scroll it (already wired above)
//   const contentNode = this._content.node;
//   contentNode.tabIndex = 0;
//   contentNode.setAttribute('role', 'log');
//   contentNode.setAttribute('aria-label', 'Console Output');
...
//   // Input panel wrapper: removed from tab order — editor inside is the real stop
//   this._input.node.tabIndex = -1;

//   // Prompt editor: tabIndex=0 so Tab reaches it after content area
// // Ensure each newly created prompt editor stays in the natural tab order
//     // and Tab key moves focus OUT instead of inserting spaces (CodeMirror default)
//     requestAnimationFrame(() => {
//       const editorNode = promptCell.editorWidget?.node;
//       if (editorNode) {
//         editorNode.tabIndex = 0;
...
//       // Find the actual CodeMirror contenteditable div and intercept Tab
//       // before CodeMirror handles it, so Tab exits the editor naturally
//       const cmContent = promptCell.node.querySelector<HTMLElement>(
//         '.cm-content, .cm-editor [contenteditable]'
//       );
//       if (cmContent) {
//         cmContent.addEventListener('keydown', (evt: KeyboardEvent) => {
//           if (evt.key === 'Escape') {
//             // Intercept Esc in capture phase before CodeMirror blurs the editor.
//             // Prevent the blur, then immediately re-focus so the focus ring
//             // stays visible — satisfying the accessibility requirement.
//             evt.preventDefault();
//             evt.stopImmediatePropagation();
//             setTimeout(() => {
//               if (!this.isDisposed) {
...
//             // Let Tab move focus to the next element outside the console
//             evt.stopImmediatePropagation();
//             // Find next focusable element after the console
//             const focusable = Array.from(
//               document.querySelectorAll<HTMLElement>(
//                 '[tabindex="0"], button:not([disabled]), a[href], input:not([disabled])'
//               )
...
//             // Shift+Tab: move focus back to content area
//             evt.stopImmediatePropagation();
//             evt.preventDefault();
//             this._content.node.focus();
//           }
//         }, true); // capture phase to beat CodeMirror
//       }
//     });

//   // Arrow key scrolling and Esc handling on the content node
//   contentNode.addEventListener('keydown', (event: KeyboardEvent) => {
//     const lineHeight = 24;
//     if (event.key === 'ArrowUp') {
//       contentNode.scrollBy({ top: -lineHeight, behavior: 'smooth' });
...
//       // Move focus into the prompt cell editor
//       this.promptCell?.editor?.focus();
//       event.preventDefault();
//     }
//   });
...
//     }
//     if (event.keyCode === 13 && !editor.hasFocus()) {
//       // Enter from content area: move focus into editor
//       event.preventDefault();
//       editor.focus();
//     } else if (event.keyCode === 27 && editor.hasFocus()) {
//       // Esc: prevent CodeMirror and any outer handler from stealing focus.
//       // stopImmediatePropagation in capture phase means no other listener
//       // on any element sees this event, so the editor keeps its focus ring.
//       event.preventDefault();
//       event.stopImmediatePropagation();
//       // Use setTimeout to re-focus AFTER CodeMirror's own Esc handling
//       // has finished running (CodeMirror blurs on Esc internally).
//       setTimeout(() => {
//         if (!this.isDisposed) {
//           editor.focus();
//         }
