...
export class CodeConsole extends Widget {
  ...
  inject(code: string, metadata: JSONObject = {}): Promise<void> {
    ...
    cell.model.setValue(code);
    for (const key of Object.keys(metadata)) {
      cell.model.ymeta.set(key, metadata[key]);
    }
    this.addCell(cell);
  ...
  private _startDrag(
    ...
    const textContent = cellModel.getValue();
  ...
  private _execute(cell: CodeCell): Promise<void> {
    const source = cell.model.getValue();
  ...
  private _handleInfo(info: KernelMessage.IInfoReplyMsg['content']): void {
    if (info.status !== 'ok') {
      this._banner!.model.setValue('Error in getting kernel banner');
      return;
    }
    this._banner!.model.setValue(info.banner);
