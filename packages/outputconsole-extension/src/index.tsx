...
function activateOutputLog(
  ...
  // on new notebook, add a command to the command palette
  app.commands.addCommand('outputconsole:open', {
    label: 'Output Log',
    execute: (args: { name: string }) => {
      const log = logRegistry.getLogger(args.name);
      const logview = new OutputLoggerView(log);
      app.shell.add(new MainAreaWidget({ content: logview }), 'main', {
        ref: '',
        mode: 'split-bottom'
      });
