# coding: utf-8
"""A tornado based Jupyter lab server based on Tornado or FastAPI."""
...
if use_server:
    ...
    ext_warn_msg = (
        "WARNING: this will delete all of your extensions, which will need to be reinstalled"
    )
    ...
    try:
        from fps_uvicorn.cli import app as main

        use_jupyverse = True
    except ImportError:
        use_jupyverse = False

    if not use_jupyverse:
        print("No server installed, please install either jupyter-server or jupyverse.")
        sys.exit()
