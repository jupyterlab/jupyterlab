...
async def run_browser(url):
    ...
    if not osp.exists(osp.join(target, "node_modules")):
        ...
        await run_async_process(["jlpm", "add", "playwright", "--network-timeout", "1000000000"], cwd=target)
...
def run_browser_sync(url):
    ...
    if not osp.exists(osp.join(target, "node_modules")):
        ...
        subprocess.call(["jlpm", "add", "playwright", "--network-timeout", "1000000000"], cwd=target)
    shutil.copy(osp.join(here, "browser-test.js"), osp.join(target, "browser-test.js"))
