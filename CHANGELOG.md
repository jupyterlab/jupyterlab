...
Themes are the recommended method for customising the JupyterLab look and feel,
while custom CSS is intended for minor personal adjustments.

To opt-in start jupyterlab with `--custom-css` flag.
The location for the `custom.css` is documented the section on [customizing the user interface](https://jupyterlab.readthedocs.io/en/latest/user/interface_customization.html#custom-css).
...
enabling multi-line completions such as those generated by large language models.
The suggestions for completions are provided by plugins implementing ``IInlineCompletionProvider`` API;
...
These links will open the corresponding file for edition (if these are in the jupyter root directory)
or open a read-only preview (if the file is outside of the root directory and the active kernel supports the debugger).
...
When cell fails during execution an error indicator will be displayed by the corresponding heading,
...
while allowing to quickly jump to a cell of a given number.
...
JupyterLab 3.6 added a notification centre which so far was only used for announcements and version update notifications (both opt-in).
