...
import { deleteDirectory, Performance, uploadContent } from './utils';
...
test.describe('Benchmark', () => {
  ...
  test.beforeAll(async ({ baseURL }) => {
    ...
    await uploadContent(baseURL, loremIpsum, 'text', `${tmpPath}/${textFile}`);
  });

  // Remove benchmark files
  test.afterAll(async ({ baseURL }) => {
    await deleteDirectory(baseURL, tmpPath);
  });
  ...
  for (const [file, sample] of parameters) {
    test(`measure ${file} - ${sample + 1}`, async ({
      ...
      const openTime = await Performance.measure(page, async () => {
      ...
      // Open text file
      const fromTime = await Performance.measure(page, async () => {
      ...
      // Switch back
      const toTime = await Performance.measure(page, async () => {
        await page.click(
      ...
      const closeTime = await Performance.measure(page, async () => {
