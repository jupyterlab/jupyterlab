...
Galata can be configured by using [command line arguments](https://playwright.dev/docs/test-cli) or using [`playwright.config.js` file](https://playwright.dev/docs/test-configuration). Full list of config options can be accessed using `jlpm playwright test --help`.
