...
If the user is new to working with software that requires working with pathing, this installation process may seem daunting. PATH, according to wikipedia, is "an environment variable on Unix-like operating systems, DOS, OS/2, and Microsoft Windows, specifying a set of directories where executable programs are located. In general, each executing process or user session has its own PATH setting." To execute either a file you create, or download as in our situation, you must have a path to that file. Just as you would double click a shortcut on a GUI, or as you would execute a file from a folder in a CLI, you must designate what you will be running and its location. What this means for the user is that the operating system requires instructions for how to use the commands it receives, and these methods differ depending on what operating system is running.
  ...
  If installing from a Windows machine, updating pathing will look a little different. After following the recommended instal process, you may find that the command to run the notebook, 'jupyter notebook', is met with the response: "'jupyter' is not recognized as an internal or external command, operable program or batch file." Windows is telling us with this message that it does not recognize the command 'jupyter' as it does not recognize it in its environment as a path variable, as a path to the executable we have in mind. To address this issue, we must locate our environment variables. From the Search Panel, first find your Control Panel. Here, locate the System folder (which may be packed together with security as 'System and Security'. Under the column of options, find 'System' and double-click to access its 'about' page. From here, you can find 'Advanced system settings'. This process leads you to the window designtaed 'System Properties' which has three sub sections labeled performance, user profiles, and startup & recovery. Under these subsections, the user will find 'Environment Variables'. Select new, and label the variable name as Path. The value of the variable, the actual location may differ for various reasons, but a standard location is: C:\Users\[UserName]\AppData\Roaming\Python\Python310\Scripts. The AppData folder is hidden and holds specific information that's not usually of concern. The roaming folder is usually used for storing data that may be transfered to different locations. From here, hit okay, and you are now able to run jupyter notebook from the command line. This application uses a web server to run, which means that the command prompt you used to run the application is now being used as a web server and closing it will also close the notebook.
